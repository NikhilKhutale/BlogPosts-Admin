...
function Write() {
  ...
  const handleClick = async (values) => {
    ...
    try {
      ...
      await axios.post("api/adminDraft/", { descr: value, ...values, date: moment(Date.now()).format("YYYY-MM-DD HH:mm:ss") })
  ...
  const onSubmit = async (values) => {
    ...
    setPostLoading(true)
    try {
      state ? await axios.put(`api/adminPost/${state.id}`, { descr: value, ...values })
        :
        await axios.post(`api/adminPost/`, { descr: value, ...values, date: moment(Date.now()).format("YYYY-MM-DD HH:mm:ss") }
        );
        setError(null)
    } catch (err) {
      setError(err)
